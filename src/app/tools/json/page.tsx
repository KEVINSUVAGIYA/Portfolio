...
function JsonTree({ data, depth = 0 }: { data: unknown; depth?: number }) {
            ...
            <div className="ml-4 border-l-2 border-indigo-500/30 pl-4 mt-1.5 space-y-1">
              {entries.map(([k, v]) => (
                <div key={k} className="flex gap-2 text-sm flex-wrap items-start">
                  <span className="text-rose-300 font-semibold flex-shrink-0">&quot;{k}&quot;:</span>
  ...
  return <div className="font-mono text-sm leading-relaxed">{renderValue(data, undefined, "root")}</div>;
}
                ...
                {parsed !== null && (
                  <div className="flex gap-1 p-1 bg-slate-800 rounded-lg border border-white/10">
                    {(["formatted", "tree"] as const).map((v) => (
                      ...
                      >
                        {v === "formatted" ? "{ } Formatted" : "🌲 Tree"}
                      </button>
                    ))}
                  </div>
                )}
                {formatted && (
                  <button onClick={copy} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-white/10 text-slate-400 hover:text-white hover:bg-white/5 transition-all">
